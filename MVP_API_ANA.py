...
# Obtendo o primeiro dia do ano e a data de hoje
hoje = date.today()
...
for codigo_pluvio in codigos_estacoes_pluvio:
    Params_pluvio = {
        ...
        "Data Inicial (yyyy-MM-dd)": primeiro_dia_ano.strftime('%Y-%m-%d'),
        "Data Final (yyyy-MM-dd)": ultimo_dia_ano.strftime('%Y-%m-%d')
    }
    ...
    if requisicao_pluvio.status_code == 200:
        ...
        if message != "sucesso":
            ...
            data_fim = ultimo_dia_ano.strftime('%Y/%m/%d')
            ...
            for estacao in estacoes_soap:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        series = resultado_raw['diffgr:diffgram']['DocumentElement']['SerieHistorica']
                        ...
                        campos = ['EstacaoCodigo', 'NivelConsistencia', 'DataHora', 'TipoMedicaoChuvas', 'Maxima', 'Total',
                                  'DiaMaxima', 'NumDiasDeChuva', 'MaximaStatus', 'TotalStatus', 'NumDiasDeChuvaStatus',
                                  'TotalAnual', 'TotalAnualStatus'] + \
                                 [f"Chuva{str(i).zfill(2)}" for i in range(1, 32)] + \
                                 [f"Chuva{str(i).zfill(2)}Status" for i in range(1, 32)] + ['DataIns']
            ...
            df.to_csv(nome_arquivo_csv_pluvio, mode='a', index=False,
                      header=not os.path.exists(nome_arquivo_csv_pluvio), encoding='utf-8')
    else:
            ...
            for estacao_fluvio in estacoes_soap_fluvio:
                ...
                if response_soap_fluvio.status_code == 200:
                    ...
                    try:
                        ...
                        filename = os.path.join(caminho_fluvio, f"fluviometria_{codigo_fluvio}.csv")
                        ...
                        print(f"Os dados da estação {estacao_fluvio} foram exportados para o arquivo '{filename}'.")

                    except KeyError:
                        print(f"Não há dados para a estação {estacao_fluvio} no período.")
                else:
